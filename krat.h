...
public:
	...
	int& Denominator() { return fDenom; }   // Требует доработки
	...
	inline KRat const operator + (KRat const& r) const;   // Возвращается const, чтобы недопустить r1+r2=r3
	...
	explicit operator double() const { return (double)fNum / fDenom; }   // Он не должен неявно вызываться
